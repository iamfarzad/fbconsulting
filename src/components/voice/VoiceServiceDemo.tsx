...
import { UnifiedVoiceUI } from './UnifiedVoiceUI';
...
export const VoiceServiceDemo = () => {
  ...
  const { speak, stopSpeaking, isSpeaking } = useVoiceService({
    showToasts: true
  });
  ...
  const handleVoiceCommand = (command: string) => {
    ...
    if (command.toLowerCase().includes('speak') || command.toLowerCase().includes('say')) {
      ...
      if (textMatch && textMatch[1]) {
        ...
        speak(textToSpeak, { rate: voiceRate, pitch: voicePitch });
      }
    } else if (command.toLowerCase().includes('hello') || command.toLowerCase().includes('hi')) {
      speak('Hello there! How can I help you today?', { rate: voiceRate, pitch: voicePitch });
    }
  };

  const handleSpeak = () => {
    if (speechText.trim()) {
      speak(speechText, { rate: voiceRate, pitch: voicePitch });
    }
  };
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col items-center justify-center space-y-6">
              ...
              <div className="relative">
                ...
                <UnifiedVoiceUI onCommand={handleVoiceCommand} />
              </div>
            </div>
          </div>

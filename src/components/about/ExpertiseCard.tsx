...
const ExpertiseCard: React.FC<ExpertiseCardProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="absolute -inset-1 bg-gradient-to-r from-primary/20 via-primary/5 to-secondary/20 rounded-xl blur opacity-0 group-hover:opacity-70 transition duration-500 group-hover:duration-200 animate-gradient-shift" />
      ...
      <div className="relative z-10">
          ...
          <div className="grid grid-cols-2 gap-4 mt-6 p-4 rounded-lg bg-white/5 backdrop-blur-sm border border-white/10">
            {Object.entries(stats).map(([key, value], index) => (
              <div key={index} className="text-center">
                <div className="text-lg font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary to-primary/70">{value}</div>
        ...
        <button className="mt-4 group/btn flex items-center text-sm font-medium text-primary hover:text-primary/80 transition-colors">
          <span>{learnMore}</span>

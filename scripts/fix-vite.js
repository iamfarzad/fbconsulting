...
console.log(`📦 Vite package ${viteExists ? 'is installed' : 'is not installed'}`);

// Check for package.json
const packageJsonPath = path.join(process.cwd(), 'package.json');
const packageJsonExists = fs.existsSync(packageJsonPath);

console.log(`📄 package.json ${packageJsonExists ? 'exists' : 'does not exist'}`);

if (packageJsonExists) {
  try {
    const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8'));
    console.log('📊 Dependencies check:');
    console.log(`  - Vite listed in dependencies: ${packageJson.dependencies && packageJson.dependencies.vite ? 'Yes' : 'No'}`);
    console.log(`  - Vite listed in devDependencies: ${packageJson.devDependencies && packageJson.devDependencies.vite ? 'Yes' : 'No'}`);
  } catch (error) {
